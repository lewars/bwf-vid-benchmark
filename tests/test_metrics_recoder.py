...
def test_metrics_recorder_initialization(tmp_path: Path):
    ...
    try:
        ...
        pytest.fail(
            f"MetricsRecorder initialization failed to create directory: {e}"
        )


def test_metrics_recorder_initialization_dir_already_exists(
    tmp_path: Path,
):
    ...
    try:
        ...
        pytest.fail(
            f"MetricsRecorder initialization failed when directory exists: {e}"
        )
...
def test_record_detailed_metrics_success(
    ...
    try:
        ...
        assert (
            data["generation_time_secs"]
            == completed_test_metrics.generation_time_secs
        )
        # Check nested PeakResourceUsage
        assert (
            data["peak_resources"]["peak_vram_mb"]
            == completed_test_metrics.peak_resources.peak_vram_mb
        )
        assert (
            data["peak_resources"]["peak_ram_mb"]
            == completed_test_metrics.peak_resources.peak_ram_mb
        )
        # Check Path conversion
        assert data["output_video_path"] == str(
            completed_test_metrics.output_video_path
        )
...
def test_record_detailed_metrics_invalid_subdir(
    tmp_path: Path, completed_test_metrics: TestMetrics
):
    ...
    with pytest.raises(
        ValueError, match="test_case_subdir must be a non-empty string"
    ):
        recorder.record_detailed_metrics(
            metrics=completed_test_metrics, test_case_subdir=""
        )
    with pytest.raises(
        ValueError, match="test_case_subdir must be a non-empty string"
    ):
...
def test_compile_summary_csv_success(
    ...
    all_metrics_list = [
        completed_test_metrics,
        failed_test_metrics,
        minimal_completed_test_metrics,
    ]
    ...
    try:
        ...
        assert df.shape == (
            3,
            len(recorder._SUMMARY_CSV_COLUMNS),
        )  # 3 rows, expected columns
        ...
        assert (
            record1["generation_time_secs"]
            == completed_test_metrics.generation_time_secs
        )
        assert (
            record1["peak_vram_mb"]
            == completed_test_metrics.peak_resources.peak_vram_mb
        )
        assert (
            record1["peak_ram_mb"]
        ...
        assert record1["output_video_path"] == str(
            completed_test_metrics.output_video_path
        )
        ...
        record3 = df.iloc[2]
        assert (
            record3["test_case_id"]
            == minimal_completed_test_metrics.test_case_id
        )
...
def test_compile_summary_csv_empty_list(tmp_path: Path):
    ...
    with pytest.raises(
        ValueError,
        match="Cannot compile summary CSV: 'all_metrics' list is empty",
    ):
        recorder.compile_summary_csv(all_metrics=[])


def test_compile_summary_csv_invalid_list_type(
    tmp_path: Path, completed_test_metrics: TestMetrics
):
    ...
    invalid_list = [
        completed_test_metrics,
        {"not": "a TestMetrics object"},
    ]

    with pytest.raises(
        ValueError, match="must be a list of TestMetrics objects"
    ):
