...
def test_load_valid_yaml(
    ...
    yaml_content = {
        "test_cases": [valid_test_case_data, valid_test_case_data_minimal]
    }
    ...
    assert loaded_cases[0].duration_secs == float(
        valid_test_case_data["duration_secs"]
    )
    ...
    assert (
        loaded_cases[1].model_name == valid_test_case_data_minimal["model_name"]
    )
    assert loaded_cases[1].prompt == valid_test_case_data_minimal["prompt"]
    assert (
        loaded_cases[1].resolution == valid_test_case_data_minimal["resolution"]
    )
    assert loaded_cases[1].duration_secs == float(
        valid_test_case_data_minimal["duration_secs"]
    )  # Check conversion
...
def test_load_yaml_missing_test_cases_key(
    ...
    with pytest.raises(
        TestCaseLoadError,
        match="must contain a top-level 'test_cases' key",
    ):
        load_test_cases_from_yaml(file_path)
...
def test_load_yaml_test_cases_not_a_list(
    ...
    with pytest.raises(
        TestCaseLoadError, match="'test_cases' key.*must contain a list"
    ):
        load_test_cases_from_yaml(file_path)
...
def test_load_yaml_test_case_validation_error(
    ...
    with pytest.raises(
        TestCaseLoadError,
        match="Error validating test case #1.*fps must be a positive integer",
    ):
        load_test_cases_from_yaml(file_path)
...
def test_load_yaml_test_case_missing_required_key(
    ...
    with pytest.raises(
        TestCaseLoadError,
        match="Error validating test case #1.*__init__.*missing.*required positional argument: 'prompt'",
    ):
        load_test_cases_from_yaml(file_path)


def test_load_yaml_empty_test_cases_list(
    yaml_file_creator: Callable[[str, Dict[str, Any]], Path], capsys
):
    ...
    assert loaded_cases == []
    assert (
        "Warning: No test case definitions found" in captured.err
        or "Warning: No test case definitions found" in captured.out
    )  # Check stdout/stderr
...
def test_load_yaml_empty_file(tmp_path: Path, capsys):
    ...
    assert loaded_cases == []
    assert (
        "Warning: Test case file is empty" in captured.err
        or "Warning: Test case file is empty" in captured.out
    )
...
def test_load_yaml_handles_null_test_cases(
    tmp_path: Path, capsys  # Need tmp_path to write raw text
):
    ...
    assert loaded_cases == []
    assert (
        "Warning: 'test_cases' key is null" in captured.err
        or "Warning: 'test_cases' key is null" in captured.out
    )


def test_load_of_non_exist_yaml_file(
    tmp_path: Path,
):
    """Test loading non existing YAML file."""
    file_path = tmp_path / "/etc/shadow"

    with pytest.raises(IOError, match=r"Could not read file .*"):
        load_test_cases_from_yaml(file_path)
