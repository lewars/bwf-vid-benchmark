...
def test_analyzer_init_success(
    ...
    try:
        ...
        pd.testing.assert_frame_equal(analyzer.data, mock_df) # Verify data loaded
...
@patch('analyzer.pd.read_csv')
def test_analyzer_init_file_not_found(mock_read_csv: MagicMock, mock_csv_path: Path):
...
@patch('analyzer.pd.read_csv')
def test_analyzer_init_missing_columns(mock_read_csv: MagicMock, mock_csv_path: Path):
    """Test initialization failure when required columns are missing."""
    # Create DataFrame missing 'peak_vram_mb'
    faulty_data = [
        {'test_case_id': 't1', 'status': 'completed', 'generation_time_secs': 10, 'peak_ram_mb': 100}
    ]
...
def test_plot_metric_barplot(
    ...
    mock_title: MagicMock, mock_xlabel: MagicMock, mock_ylabel: MagicMock,
    mock_xticks: MagicMock, mock_tight_layout: MagicMock,
    ...
    mock_read_csv.return_value = mock_df
    mock_csv_path.touch()
    analyzer = ResultsAnalyzer(csv_path=mock_csv_path) # Init loads the mock data
    ...
    mock_xlabel.assert_called_once_with('Test Case ID') # Assuming default xlabel
...
@patch('analyzer.ResultsAnalyzer.plot_metric_barplot') # Patch the method within the class
...
def test_plot_metric_barplot_save_error(
    ...
    mock_savefig.assert_called_once_with(output_file) # Ensure savefig was attempted
