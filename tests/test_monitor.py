...
@patch('monitor.pynvml', create=True) # Patch pynvml import within monitor module
def test_monitor_init_defaults(mock_pynvml_import, mock_Event, mock_getpid):
    ...
    mock_pynvml_import.nvmlDeviceGetHandleByIndex.return_value = MagicMock(name="Handle")
...
def test_monitor_init_custom_values(mock_pynvml_import, mock_Event, mock_getpid):
...
@patch('monitor.threading.Event')
@patch('monitor.pynvml', create=True)
def test_monitor_init_pynvml_init_fails(mock_pynvml_import, mock_Event, mock_getpid, mock_log):
    """Test initialization when pynvml.nvmlInit fails."""
    mock_pynvml_import.NVMLError = type('MockNVMLError', (Exception,), {}) # Create mock error type
    mock_pynvml_import.nvmlInit.side_effect = mock_pynvml_import.NVMLError("Init Failed")
...
@patch('monitor.threading.Event')
@patch('monitor.pynvml', create=True)
def test_monitor_init_pynvml_get_handle_fails(mock_pynvml_import, mock_Event, mock_getpid, mock_log):
    ...
    mock_pynvml_import.nvmlDeviceGetHandleByIndex.side_effect = mock_pynvml_import.NVMLError("Handle Failed")
    ...
    mock_pynvml_import.nvmlInit.assert_called_once()
    mock_pynvml_import.nvmlDeviceGetHandleByIndex.assert_called_once_with(0)
    mock_log.warning.assert_any_call(f"Failed to get NVML handle for GPU 0: Handle Failed")
...
def test_monitor_start_stop_loop(
    mock_getpid, mock_pynvml_import, mock_psutil_import, mock_Thread, mock_sleep,
    mock_pynvml, mock_psutil # Use fixtures for mock objects
):
    ...
    mock_pynvml_import.nvmlDeviceGetHandleByIndex = pynvml_module_mock.nvmlDeviceGetHandleByIndex
    mock_pynvml_import.nvmlDeviceGetMemoryInfo = pynvml_module_mock.nvmlDeviceGetMemoryInfo
    ...
    nvml_mem_info_mock.used = 180 * 1024 * 1024 # 180 MB (lower than initial mock, peak stays 200)
    ...
    monitor._peak_vram_bytes = max(monitor._peak_vram_bytes, 200 * 1024 * 1024) # Initial mocked value was peak
...
def test_monitor_context_manager(mock_stop, mock_start):
    ...
    monitor = ResourceMonitor() # Doesn't matter if init fails here, just testing context
