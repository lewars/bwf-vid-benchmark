...
def test_orchestrator_setup(
    mock_HunyuanAdapter: MagicMock,
    mock_Mochi1Adapter: MagicMock,
    mock_ResourceMonitor: MagicMock,
    mock_MetricsRecorder: MagicMock,
    mock_VideoGenerator: MagicMock,
    ...
    mock_vg_instance.register_adapter.assert_any_call(
        "mochi1", mock_Mochi1Adapter
    )
    mock_vg_instance.register_adapter.assert_any_call(
        "hunyuan", mock_HunyuanAdapter
    )
    mock_MetricsRecorder.assert_called_once_with(
        base_results_dir=mock_paths["results"], run_id="mock_timestamp"
    )
...
def test_orchestrator_setup_load_failure(
    ...
    assert (
        orchestrator_instance._is_setup is False
    )  # Should not be set if setup fails
...
def test_execute_test_cases_all(
    ...
    mock_run_single.side_effect = lambda tc: TestMetrics(
        test_case_id=tc.id, status="completed"
    )
...
def test_execute_test_cases_specific_ids(
    ...
    mock_run_single.assert_has_calls(
        [call(mock_test_cases[2]), call(mock_test_cases[0])],
        any_order=True,
    )
    # Assert metrics collected match executed tests
    assert len(orchestrator_instance.all_run_metrics) == 2
    assert {m.test_case_id for m in orchestrator_instance.all_run_metrics} == {
        "t1",
        "t3",
    }


def test_execute_test_cases_requires_setup(
    orchestrator_instance: BenchmarkOrchestrator,
):
...
def test_run_single_test_success(
    ...
    mock_monitor_instance.stop.return_value = PeakResourceUsage(
        peak_vram_mb=1024.0, peak_ram_mb=2048.0
    )
    ...
    assert recorded_metrics_arg.generation_time_secs == pytest.approx(
        20.5
    )  # 120.5 - 100.0
    assert (
        recorded_metrics_arg.peak_resources
    ...
    assert call_args[1] == {
        "test_case_subdir": test_case.id
    }  # Check kwargs if used
...
def test_run_single_test_generation_fails(
    ...
    mock_monitor_instance = MagicMock()
    mock_monitor_instance.stop.return_value = PeakResourceUsage(
        peak_vram_mb=512.0, peak_ram_mb=1024.0
    )
    mock_ResourceMonitor.return_value = mock_monitor_instance
    mock_vg_instance = MagicMock(spec=VideoGenerator)
    mock_mr_instance = MagicMock(spec=MetricsRecorder)
    ...
    mock_vg_instance.generate_for_test_case.side_effect = RuntimeError(
        error_message
    )
    mock_mr_instance.run_dir_path = Path("results/mock_timestamp_run")
    ...
    assert (
        recorded_metrics_arg.generation_time_secs is None
    )  # End time wasn't reached after start
    assert (
        recorded_metrics_arg.peak_resources
        == mock_monitor_instance.stop.return_value
    )
    ...
    assert (
        error_message in recorded_metrics_arg.error_message
...
def test_compile_summary(orchestrator_instance: BenchmarkOrchestrator):
    ...
    dummy_metrics = [
        TestMetrics(test_case_id="t1", status="completed"),
        TestMetrics(test_case_id="t2", status="failed"),
    ]
