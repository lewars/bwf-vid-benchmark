...
def test_peakresourceusage_successful_initialization():
    ...
    try:
        res = PeakResourceUsage(peak_vram_mb=4096.0, peak_ram_mb=8192) # Test int RAM
        assert res.peak_vram_mb == 4096.0
        assert res.peak_ram_mb == 8192.0 # Assuming conversion or flexible typing
...
def test_testmetrics_successful_initialization_completed(valid_peak_resources):
...
def test_testmetrics_validation_invalid_test_case_id(valid_peak_resources):
    ...
    with pytest.raises(ValueError, match="test_case_id.*non-empty string"):
        TestMetrics(test_case_id="", status="completed", generation_time_secs=10.0,
                    peak_resources=valid_peak_resources, output_video_path="path", error_message=None)
    with pytest.raises(ValueError, match="test_case_id.*non-empty string"):
        TestMetrics(test_case_id=None, status="completed", generation_time_secs=10.0, # type: ignore
                    peak_resources=valid_peak_resources, output_video_path="path", error_message=None)

def test_testmetrics_validation_invalid_status(valid_peak_resources):
    """Test validation failure for invalid status."""
    with pytest.raises(ValueError, match="status must be one of"):
        TestMetrics(test_case_id="test003", status="success", generation_time_secs=10.0,
                    peak_resources=valid_peak_resources, output_video_path="path", error_message=None)

def test_testmetrics_validation_invalid_generation_time(valid_peak_resources):
    ...
    with pytest.raises(ValueError, match="generation_time_secs.*non-negative"):
        TestMetrics(test_case_id="test004", status="completed", generation_time_secs=-5.0,
                    peak_resources=valid_peak_resources, output_video_path="path", error_message=None)
    with pytest.raises(ValueError, match="generation_time_secs.*non-negative"):
        TestMetrics(test_case_id="test004", status="completed", generation_time_secs="fast", # type: ignore
                    peak_resources=valid_peak_resources, output_video_path="path", error_message=None)


def test_testmetrics_validation_invalid_peak_resources():
    """Test validation failure for invalid peak_resources type."""
    with pytest.raises(TypeError, match="peak_resources.*PeakResourceUsage instance"):
        TestMetrics(test_case_id="test005", status="completed", generation_time_secs=10.0,
                    peak_resources={"vram": 100}, output_video_path="path", error_message=None) # type: ignore
...
def test_testmetrics_validation_invalid_output_path(valid_peak_resources):
    ...
    with pytest.raises(TypeError, match="output_video_path.*string or None"):
        TestMetrics(test_case_id="test006", status="completed", generation_time_secs=10.0,
                    peak_resources=valid_peak_resources, output_video_path=123, error_message=None) # type: ignore
...
def test_testmetrics_validation_error_message_mismatch():
    ...
    with pytest.raises(ValueError, match="error_message should be None"):
        TestMetrics(test_case_id="test007", status="completed", generation_time_secs=10.0,
                    peak_resources=None, output_video_path="path", error_message="Should not be here")
    # Status is 'failed' but error_message is None
    with pytest.raises(ValueError, match="error_message.*non-empty string.*failed"):
        TestMetrics(test_case_id="test008", status="failed", generation_time_secs=None,
                    peak_resources=None, output_video_path=None, error_message=None)
    # Status is 'failed' but error_message is empty
    with pytest.raises(ValueError, match="error_message.*non-empty string.*failed"):
        TestMetrics(test_case_id="test009", status="failed", generation_time_secs=None,
                    peak_resources=None, output_video_path=None, error_message="")
